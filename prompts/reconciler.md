...
## Typed Inputs
Accepted candidates, critic reports, verifier reports, approved schema, and run context.
...
Read the JSON user input. It contains run_id, plan, accepted candidates, accepted critic reports, and accepted verifier reports.
...
- Do not invent category or field_name values. Use only approved schema values from the plan.
...
- Use the source_candidate_id whose source_span best grounds the final value.
...
- Prefer the most specific, best-grounded source span as source_candidate_id.
...
- Confidence should not exceed the support in the contributing candidates and verifier reports.
- The final value must be supported by the source_candidate_id source span and must not add outside knowledge.
