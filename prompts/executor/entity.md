...
- source_text must be copied exactly from chunk_view.text starting at start_char. End offsets and byte offsets are derived server-side from start_char and source_text — do not return them.
...
Few-shot examples:
- Valid: chunk_view.start_char=100, chunk_view.text begins with "Acme Corp acquired Beta LLC." Approved field is Counterparty.name. For "Acme Corp", use source_text="Acme Corp", start_char=100.
