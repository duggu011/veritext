...
- If an approved field asks for a statement-like value such as summary, statement, description, or condition, use the source sentence or standalone clause rather than a paraphrase.
- If an approved field asks for a qualifier or static attribute such as notable_qualifier or an asset_detail operational profile, use the tight source phrase that states that qualifier or attribute.
...
- For qualifier or attribute fields such as notable_qualifier or asset_detail, choose the shortest exact phrase or clause that carries the qualifier/detail role; use a full sentence only when the whole event sentence is the asset detail.
...
- Valid event-detail span: if approved field is CorporateEvent.asset_detail and source states "The company opened Line 4 after completing safety certification.", extract the full sentence or the standalone clause that exactly matches the approved event detail, including closing punctuation.
...
- Valid effective-date event context: if approved field is PersonnelChange.effective_date and source says "effective at the June 18, 2026 Annual Meeting", select "June 18, 2026 Annual Meeting" because the named meeting is part of the effective timing.
