...
- Category descriptions for reusable categories must define the source-backed fact class, not a closed list of document examples. CorporateEvent covers source-backed significant business or corporate events such as acquisitions, approvals, facility commencements or operation starts, transactions, and other stated corporate actions; do not reserve CorporateEvent for one named transaction when another source-backed event matches the approved fields.
- Use OperationalMetric, distinct from FinancialMetric, when the source states non-financial operational performance such as capacity factor, generation or throughput in physical units (e.g. gigawatt-hours), utilization, fleet output, plant availability, or per-facility production. Do not collapse these into FinancialMetric merely because the document also reports financial metrics; both categories should appear when both kinds of facts are present.
- Include an optional OperationalMetric.facility field when an operational metric is tied to a named facility or asset. Do not move that facility role to CorporateEvent merely because the same sentence also states a facility commencement or other event.
...
- Avoid field synonym drift: use change_pct rather than change_percentage, exposure_pct rather than exposure_percentage, event_date rather than announcement_date unless the source distinguishes a separate announcement date, guidance_value rather than forecast_value for values inside forward guidance, effective_date rather than event_date for the date a regulatory rate change or personnel appointment takes effect when the source distinguishes effective timing from announcement or board-approval timing, and forecast_value rather than target_value when the source labels the comparator a forecast (for example "$88.0 million forecast" for an EBITDA result) rather than a forward target.
...
- When the field name is facility, describe it as the bare source-stated facility or asset name. Do not require location unless the field name or source role explicitly requires facility plus location.
- For fields ending in _type, including change_type and event_type, prefer concise source-traced labels. Noun-form normalization is valid when the supporting source words state the same action or type, and the provenance span remains on those source words.
...
- Metric facts should preserve metric_name, value, period, comparison or change, prior/target/forecast values, and notable_qualifier when those roles are stated.
- Guidance facts should preserve speaker, guidance date/period, guided metric, guidance value/range, target status, target_date, condition, and other qualifiers when stated.
...
- Avoid role collapse such as deadline for expected close date, effective_date for announcement date, value for forecast value, rate for prior/new regulatory rates, party or person for guidance speaker, period for guidance target date, conditions for one condition, summary for event type, summary for a metric notable_qualifier, or person for personnel role when the source distinguishes those roles.
