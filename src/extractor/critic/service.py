    ...
    async with semaphore:
        result = await llm_client.complete_structured(
            StructuredLLMRequest(
                ...
                user_content=CriticBatchStageInput(
                    run_id=plan.run_id,
                    plan=plan,
                    chunk=chunk,
                    candidates=batch,
            ...
            output_model=CriticBatchReviewPayload,
            audit_store=audit_store,
        )

    reports_by_candidate: dict[str, CriticBatchReportPayload] = {}
    for report_payload in result.output.reports:
        # First report wins — the model occasionally repeats a candidate_id.
...
def _build_report(
    *,
    ...
    payload: CriticBatchReportPayload,
    critic_call_id: str,
) -> tuple[CriticReport, LensCandidate | None, list[RejectionReason]]:
    corrected_candidate, structural_reasons = _materialize_correction(
        raw=payload.corrected_candidate,
        chunk=chunk,
    )
    validation_reasons: list[RejectionReason] = list(structural_reasons)
    ...
    accepted = payload.accepted and not validation_reasons
    ...
    report = CriticReport(
        report_id=_stable_report_id(
            ...
            plausibility_score=payload.plausibility_score,
            accepted=accepted,
            issues=issues,
...
def _materialize_correction(
    *,
    raw: RawCorrectedCandidate | None,
    chunk: Chunk,
) -> tuple[LensCandidate | None, list[RejectionReason]]:
    """Convert the relaxed tool-input correction into a strict LensCandidate.

    The LLM returns char offset + exact text; bytes and end_char are derived
    from the chunk. If the result still violates LensCandidate invariants
    (text/chunk mismatch, etc.), we surface a typed RejectionReason rather
    than letting Pydantic crash the run.
    """
    if raw is None:
        return None, []

    span = raw.source_span
    relative_start = span.start_char - chunk.start_char
    relative_end = relative_start + len(span.text)
    if (
        relative_start < 0
        or relative_end > len(chunk.text)
        or chunk.text[relative_start:relative_end] != span.text
    ):
        return None, [
            RejectionReason(
                code="invented_span",
                message=(
                    ...
                    f"slice at start_char {span.start_char}."
                ),
            )
        ]
