...
async def ingest_document(
    ...
    *,
    ...
    text, page_map = await _extract_text_and_page_map(resolved_path, document_format, source_bytes)
...
async def _extract_text_and_page_map(
    source_path: Path,
    ...
    if document_format in ("plain_text", "markdown"):
        ...
        return text, (_single_page_span(text),)
...
def _extract_pdf_text_and_page_map(source_path: Path) -> tuple[str, tuple[PageSpan, ...]]:
    try:
...
def _join_pages(page_texts: list[str]) -> tuple[str, tuple[PageSpan, ...]]:
    text_parts: list[str] = []
    ...
    return "".join(text_parts), tuple(page_map)
